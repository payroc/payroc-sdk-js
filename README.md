...
If you wish to use the SDK against a custom URL, such as a mock API server, you can provide a custom `PayrocEnvironmentUrls` to the `PayrocClient` constructor:

```typescript
import { PayrocClient, PayrocEnvironmentUrls } from "payroc";
...
const pager = await client.cardPayments.payments.list({ processingTerminalId: "1234001" });
...
```typescript
const pager = await client.cardPayments.payments.list({});
...
const request: Payroc.cardPayments.ListPaymentsRequest = {
    processingTerminalId: "1234001"
};
```
...
```typescript
const payment = await client.cardPayments.payments.retrieve({ paymentId: "12345" });
...
if (payment.paymentMethod.type === "card") {
    ...
    console.log(`Token: ${payment.paymentMethod.token}`);
} else if (payment.paymentMethod.type === "digitalWallet") {
    // TypeScript knows this is a digital wallet payment method
    console.log(`Service provider: ${payment.paymentMethod.serviceProvider}`);
}
```
...
switch (payment.paymentMethod.type) {
    case "card":
        ...
        break;
    case "secureToken":
        console.log(`Token payment: ${payment.paymentMethod.token}`);
        break;
    case "digitalWallet":
        console.log(`Wallet payment: ${payment.paymentMethod.serviceProvider}`);
        break;
    case "singleUseToken":
...
```typescript
const { data, rawResponse } = await client.cardPayments.payments.create(...).withRawResponse();
...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=https%3A%2F%2Fgithub.com%2Fpayroc%2Fpayroc-sdk-js)

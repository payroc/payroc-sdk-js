// This file was auto-generated by Fern from our API Definition.

import * as Payroc from "../../../src/api/index";
import { PayrocClient } from "../../../src/Client";
import { mockServerPool } from "../../mock-server/MockServerPool";
import { mockBearer } from "../mockAuth";

describe("FundingAccountsClient", () => {
    test("list (1)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = {
            limit: 2,
            count: 2,
            hasMore: true,
            links: [
                {
                    rel: "previous",
                    method: "get",
                    href: "https://api.payroc.com/v1/funding-accounts?before=123&limit=2",
                },
                { rel: "next", method: "get", href: "https://api.payroc.com/v1/funding-accounts?after=124&limit=2" },
            ],
            data: [
                {
                    fundingAccountId: 123,
                    createdDate: "2024-07-02T15:30:00Z",
                    lastModifiedDate: "2024-07-02T15:30:00Z",
                    status: "approved",
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "Jane Doe",
                    paymentMethods: [
                        { type: "ach", value: { routingNumber: "123456789", accountNumber: "1234567890" } },
                    ],
                    metadata: { yourCustomField: "abc123" },
                    links: [
                        {
                            rel: "parent",
                            method: "get",
                            href: "https://api.payroc.com/v1/processing-accounts/4525644354",
                        },
                    ],
                },
                {
                    fundingAccountId: 124,
                    createdDate: "2024-07-02T15:30:00Z",
                    lastModifiedDate: "2024-07-02T15:30:00Z",
                    status: "pending",
                    type: "checking",
                    use: "debit",
                    nameOnAccount: "Jane Doe",
                    paymentMethods: [
                        { type: "ach", value: { routingNumber: "123456789", accountNumber: "1234567890" } },
                    ],
                    metadata: { yourCustomField: "abc123" },
                    links: [{ rel: "parent", method: "get", href: "https://api.payroc.com/v1/funding-recipients/234" }],
                },
            ],
        };
        server
            .mockEndpoint({ once: false })
            .get("/funding-accounts")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const expected = {
            limit: 2,
            count: 2,
            hasMore: true,
            links: [
                {
                    rel: "previous",
                    method: "get",
                    href: "https://api.payroc.com/v1/funding-accounts?before=123&limit=2",
                },
                {
                    rel: "next",
                    method: "get",
                    href: "https://api.payroc.com/v1/funding-accounts?after=124&limit=2",
                },
            ],
            data: [
                {
                    fundingAccountId: 123,
                    createdDate: "2024-07-02T15:30:00Z",
                    lastModifiedDate: "2024-07-02T15:30:00Z",
                    status: "approved",
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "Jane Doe",
                    paymentMethods: [
                        {
                            type: "ach",
                            value: {
                                routingNumber: "123456789",
                                accountNumber: "1234567890",
                            },
                        },
                    ],
                    metadata: {
                        yourCustomField: "abc123",
                    },
                    links: [
                        {
                            rel: "parent",
                            method: "get",
                            href: "https://api.payroc.com/v1/processing-accounts/4525644354",
                        },
                    ],
                },
                {
                    fundingAccountId: 124,
                    createdDate: "2024-07-02T15:30:00Z",
                    lastModifiedDate: "2024-07-02T15:30:00Z",
                    status: "pending",
                    type: "checking",
                    use: "debit",
                    nameOnAccount: "Jane Doe",
                    paymentMethods: [
                        {
                            type: "ach",
                            value: {
                                routingNumber: "123456789",
                                accountNumber: "1234567890",
                            },
                        },
                    ],
                    metadata: {
                        yourCustomField: "abc123",
                    },
                    links: [
                        {
                            rel: "parent",
                            method: "get",
                            href: "https://api.payroc.com/v1/funding-recipients/234",
                        },
                    ],
                },
            ],
        };
        const page = await client.funding.fundingAccounts.list({
            before: "2571",
            after: "8516",
            limit: 1,
        });
        expect(expected.data).toEqual(page.data);
    });

    test("list (2)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = {
            limit: 10,
            count: 0,
            hasMore: false,
            links: [
                { rel: "previous", method: "get", href: "<uri>" },
                { rel: "next", method: "get", href: "<uri>" },
            ],
            data: [
                {
                    fundingAccountId: 123,
                    createdDate: "2024-07-02T12:00:00Z",
                    lastModifiedDate: "2024-07-02T12:00:00Z",
                    status: "approved",
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "Jane Doe",
                    paymentMethods: [
                        { type: "ach", value: { routingNumber: "123456789", accountNumber: "1234567890" } },
                    ],
                    metadata: { yourCustomField: "abc123" },
                    links: [{ rel: "previous", method: "get", href: "<uri>" }],
                },
            ],
        };
        server
            .mockEndpoint({ once: false })
            .get("/funding-accounts")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const expected = {
            limit: 10,
            count: 0,
            hasMore: false,
            links: [
                {
                    rel: "previous",
                    method: "get",
                    href: "<uri>",
                },
                {
                    rel: "next",
                    method: "get",
                    href: "<uri>",
                },
            ],
            data: [
                {
                    fundingAccountId: 123,
                    createdDate: "2024-07-02T12:00:00Z",
                    lastModifiedDate: "2024-07-02T12:00:00Z",
                    status: "approved",
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "Jane Doe",
                    paymentMethods: [
                        {
                            type: "ach",
                            value: {
                                routingNumber: "123456789",
                                accountNumber: "1234567890",
                            },
                        },
                    ],
                    metadata: {
                        yourCustomField: "abc123",
                    },
                    links: [
                        {
                            rel: "previous",
                            method: "get",
                            href: "<uri>",
                        },
                    ],
                },
            ],
        };
        const page = await client.funding.fundingAccounts.list({
            before: "2571",
            after: "8516",
            limit: 1,
        });
        expect(expected.data).toEqual(page.data);
    });

    test("list (3)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint({ once: false })
            .get("/funding-accounts")
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.list();
        }).rejects.toThrow(Payroc.BadRequestError);
    });

    test("list (4)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint({ once: false })
            .get("/funding-accounts")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.list();
        }).rejects.toThrow(Payroc.UnauthorizedError);
    });

    test("list (5)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { key: "value" };
        server
            .mockEndpoint({ once: false })
            .get("/funding-accounts")
            .respondWith()
            .statusCode(403)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.list();
        }).rejects.toThrow(Payroc.ForbiddenError);
    });

    test("list (6)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint({ once: false })
            .get("/funding-accounts")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.list();
        }).rejects.toThrow(Payroc.NotFoundError);
    });

    test("list (7)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint({ once: false })
            .get("/funding-accounts")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.list();
        }).rejects.toThrow(Payroc.InternalServerError);
    });

    test("retrieve (1)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = {
            fundingAccountId: 123,
            createdDate: "2024-07-02T15:30:00Z",
            lastModifiedDate: "2024-07-02T15:30:00Z",
            status: "pending",
            type: "checking",
            use: "credit",
            nameOnAccount: "Jane Doe",
            paymentMethods: [{ type: "ach", value: { routingNumber: "123456789", accountNumber: "1234567890" } }],
            metadata: { yourCustomField: "abc123" },
            links: [{ rel: "parent", method: "get", href: "https://api.payroc.com/v1/processing-accounts/4525644354" }],
        };
        server
            .mockEndpoint()
            .get("/funding-accounts/1")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.funding.fundingAccounts.retrieve({
            fundingAccountId: 1,
        });
        expect(response).toEqual({
            fundingAccountId: 123,
            createdDate: "2024-07-02T15:30:00Z",
            lastModifiedDate: "2024-07-02T15:30:00Z",
            status: "pending",
            type: "checking",
            use: "credit",
            nameOnAccount: "Jane Doe",
            paymentMethods: [
                {
                    type: "ach",
                    value: {
                        routingNumber: "123456789",
                        accountNumber: "1234567890",
                    },
                },
            ],
            metadata: {
                yourCustomField: "abc123",
            },
            links: [
                {
                    rel: "parent",
                    method: "get",
                    href: "https://api.payroc.com/v1/processing-accounts/4525644354",
                },
            ],
        });
    });

    test("retrieve (2)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .get("/funding-accounts/1")
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.retrieve({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.BadRequestError);
    });

    test("retrieve (3)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .get("/funding-accounts/1")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.retrieve({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.UnauthorizedError);
    });

    test("retrieve (4)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { key: "value" };
        server
            .mockEndpoint()
            .get("/funding-accounts/1")
            .respondWith()
            .statusCode(403)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.retrieve({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.ForbiddenError);
    });

    test("retrieve (5)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .get("/funding-accounts/1")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.retrieve({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.NotFoundError);
    });

    test("retrieve (6)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .get("/funding-accounts/1")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.retrieve({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.InternalServerError);
    });

    test("update (1)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });
        const rawRequestBody = {
            type: "checking",
            use: "credit",
            nameOnAccount: "Jane Doe",
            paymentMethods: [{ type: "ach" }],
        };

        server.mockEndpoint().put("/funding-accounts/1").jsonBody(rawRequestBody).respondWith().statusCode(200).build();

        const response = await client.funding.fundingAccounts.update({
            fundingAccountId: 1,
            body: {
                type: "checking",
                use: "credit",
                nameOnAccount: "Jane Doe",
                paymentMethods: [
                    {
                        type: "ach",
                    },
                ],
            },
        });
        expect(response).toEqual(undefined);
    });

    test("update (2)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });
        const rawRequestBody = {
            type: "checking",
            use: "credit",
            nameOnAccount: "nameOnAccount",
            paymentMethods: [{ type: "ach" }, { type: "ach" }],
        };
        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .put("/funding-accounts/1")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.update({
                fundingAccountId: 1,
                body: {
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "nameOnAccount",
                    paymentMethods: [
                        {
                            type: "ach",
                        },
                        {
                            type: "ach",
                        },
                    ],
                },
            });
        }).rejects.toThrow(Payroc.BadRequestError);
    });

    test("update (3)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });
        const rawRequestBody = {
            type: "checking",
            use: "credit",
            nameOnAccount: "nameOnAccount",
            paymentMethods: [{ type: "ach" }, { type: "ach" }],
        };
        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .put("/funding-accounts/1")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.update({
                fundingAccountId: 1,
                body: {
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "nameOnAccount",
                    paymentMethods: [
                        {
                            type: "ach",
                        },
                        {
                            type: "ach",
                        },
                    ],
                },
            });
        }).rejects.toThrow(Payroc.UnauthorizedError);
    });

    test("update (4)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });
        const rawRequestBody = {
            type: "checking",
            use: "credit",
            nameOnAccount: "nameOnAccount",
            paymentMethods: [{ type: "ach" }, { type: "ach" }],
        };
        const rawResponseBody = { key: "value" };
        server
            .mockEndpoint()
            .put("/funding-accounts/1")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(403)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.update({
                fundingAccountId: 1,
                body: {
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "nameOnAccount",
                    paymentMethods: [
                        {
                            type: "ach",
                        },
                        {
                            type: "ach",
                        },
                    ],
                },
            });
        }).rejects.toThrow(Payroc.ForbiddenError);
    });

    test("update (5)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });
        const rawRequestBody = {
            type: "checking",
            use: "credit",
            nameOnAccount: "nameOnAccount",
            paymentMethods: [{ type: "ach" }, { type: "ach" }],
        };
        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .put("/funding-accounts/1")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.update({
                fundingAccountId: 1,
                body: {
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "nameOnAccount",
                    paymentMethods: [
                        {
                            type: "ach",
                        },
                        {
                            type: "ach",
                        },
                    ],
                },
            });
        }).rejects.toThrow(Payroc.NotFoundError);
    });

    test("update (6)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });
        const rawRequestBody = {
            type: "checking",
            use: "credit",
            nameOnAccount: "nameOnAccount",
            paymentMethods: [{ type: "ach" }, { type: "ach" }],
        };
        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .put("/funding-accounts/1")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(406)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.update({
                fundingAccountId: 1,
                body: {
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "nameOnAccount",
                    paymentMethods: [
                        {
                            type: "ach",
                        },
                        {
                            type: "ach",
                        },
                    ],
                },
            });
        }).rejects.toThrow(Payroc.NotAcceptableError);
    });

    test("update (7)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });
        const rawRequestBody = {
            type: "checking",
            use: "credit",
            nameOnAccount: "nameOnAccount",
            paymentMethods: [{ type: "ach" }, { type: "ach" }],
        };
        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .put("/funding-accounts/1")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.update({
                fundingAccountId: 1,
                body: {
                    type: "checking",
                    use: "credit",
                    nameOnAccount: "nameOnAccount",
                    paymentMethods: [
                        {
                            type: "ach",
                        },
                        {
                            type: "ach",
                        },
                    ],
                },
            });
        }).rejects.toThrow(Payroc.InternalServerError);
    });

    test("delete (1)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        server.mockEndpoint().delete("/funding-accounts/1").respondWith().statusCode(200).build();

        const response = await client.funding.fundingAccounts.delete({
            fundingAccountId: 1,
        });
        expect(response).toEqual(undefined);
    });

    test("delete (2)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .delete("/funding-accounts/1")
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.delete({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.BadRequestError);
    });

    test("delete (3)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .delete("/funding-accounts/1")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.delete({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.UnauthorizedError);
    });

    test("delete (4)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { key: "value" };
        server
            .mockEndpoint()
            .delete("/funding-accounts/1")
            .respondWith()
            .statusCode(403)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.delete({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.ForbiddenError);
    });

    test("delete (5)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .delete("/funding-accounts/1")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.delete({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.NotFoundError);
    });

    test("delete (6)", async () => {
        const server = mockServerPool.createServer();
        mockBearer(server);

        const client = new PayrocClient({
            maxRetries: 0,
            apiKey: "x-api-key",
            environment: { api: server.baseUrl, identity: server.baseUrl },
        });

        const rawResponseBody = { type: "type", title: "title", status: 1, detail: "detail" };
        server
            .mockEndpoint()
            .delete("/funding-accounts/1")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.funding.fundingAccounts.delete({
                fundingAccountId: 1,
            });
        }).rejects.toThrow(Payroc.InternalServerError);
    });
});
